...
# Check event 10031 timing
data_dict = pd.read_csv('data_dictionary.csv')
event_10031 = data_dict[data_dict['Event #'] == 10031].iloc[0]

print("Event 10031 from data dictionary:")
print(f"Fault_Timestamp: {event_10031['Fault_Timestamp']}ms")
print(f"INV_power: {event_10031['INV_power']}W")
print(f"RLC_power: {event_10031['RLC_power']}W")

# Load and process the CSV
csv_file = 'SEL_events07232025_csv/CEV_10031_R.csv'
df = af.raw_csv_to_df_add_rms(csv_file)

fault_time_s = event_10031['Fault_Timestamp'] / 1000  # Convert to seconds
...
plt.axvline(x=event_10031['Fault_Timestamp'], color='black', linestyle='--', linewidth=2, label=f'Fault Time ({event_10031["Fault_Timestamp"]}ms)')

plt.xlabel('Time (ms)')
plt.ylabel('Voltage (V)')
plt.title('Event 10031 - First 1000ms Voltage Waveforms')
plt.legend()
...
plt.title('Event 10031 - First 1000ms Current Waveforms')
plt.legend()
plt.grid(True, alpha=0.3)

plt.tight_layout()
plt.savefig('event_10031_first_1000ms.png', dpi=300, bbox_inches='tight')
plt.show()

print(f"\nPlot saved as: event_10031_first_1000ms.png")
